...
import { Download, Lock, LogOut, LayoutDashboard, TrendingUp, PieChart as PieIcon } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const downloadExcel = () => {
    const formattedData = data.map(item => ({
      ...
      "Dining": item.food, "Stay": item.stay, "Conference": item.conference, "Campus": item.campus, "Activity": item.activities,
  ...
  if (!isAuthenticated) {
    return (
      <div className={styles.container}>
        <motion.div className={styles.card} initial={{ scale: 0.9 }} animate={{ scale: 1 }} style={{ maxWidth: '400px', textAlign: 'center', padding: '40px' }}>
          <Lock size={48} color="#1e3a8a" style={{ marginBottom: '20px' }} />
          <h2 className={styles.title} style={{ fontSize: '1.8rem', color: '#0f172a' }}>Admin Panel</h2>
          <form onSubmit={handleLogin}>
            <input type="password" placeholder="Enter PIN" className={styles.textArea} style={{ minHeight: '50px', margin: '20px 0', textAlign: 'center' }} value={password} onChange={(e) => setPassword(e.target.value)} />
  ...
  // --- RENDER: DASHBOARD ---
  return (
    <div className={styles.container} style={{ alignItems: 'flex-start', paddingBottom: '50px' }}>
      <div style={{ width: '100%', maxWidth: '1200px' }}>
        
        {/* HEADER */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', color: 'white' }}>
          <div>
            <h1 className={styles.title} style={{ fontSize: '2.2rem', textShadow: '0 2px 4px rgba(0,0,0,0.3)' }}>Analytics Overview</h1>
            <p className={styles.subtitle}>Real-time feedback insights</p>
          </div>
          <div style={{ display: 'flex', gap: '10px' }}>
            <button onClick={downloadExcel} className={styles.button} style={{ margin: 0, width: 'auto', padding: '12px 24px', background: '#10b981' }}>
              <Download size={18} /> Export Excel
            </button>
            <button onClick={() => setIsAuthenticated(false)} className={styles.button} style={{ margin: 0, width: 'auto', padding: '12px', background: 'rgba(255,255,255,0.2)' }}>
              <LogOut size={18} />
            </button>
          </div>
        </div>
        ...
        <div style={adminStyles.grid}>
          ...
          <motion.div style={adminStyles.statCard} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
            <div style={adminStyles.statLabel}>Total Responses</div>
            <div style={adminStyles.statValue}>{data.length}</div>
            <div style={{ fontSize: '0.8rem', color: '#94a3b8' }}>Submissions</div>
          </motion.div>
        </div>
        ...
        <div style={adminStyles.chartsRow}>
          ...
          <div style={adminStyles.chartContainer}>
            <div style={adminStyles.chartTitle}><TrendingUp size={20} /> Category Performance</div>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={metrics.averages}>
                <CartesianGrid strokeDasharray="3 3" opacity={0.3} />
                <XAxis dataKey="name" />
                <YAxis domain={[0, 5]} />
                <Tooltip contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 5px 15px rgba(0,0,0,0.1)' }} />
          ...
          <div style={adminStyles.chartContainer}>
            <div style={adminStyles.chartTitle}><PieIcon size={20} /> Overall Rating Distribution</div>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
        ...
        <div style={adminStyles.chartContainer}>
          <div style={adminStyles.chartTitle}><LayoutDashboard size={20} /> Recent Comments</div>
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {data.slice(0, 5).map((item, i) => (
              <div key={i} style={{ padding: '15px 0', borderBottom: '1px solid #e2e8f0' }}>
                ...
                <p style={{ margin: 0, color: '#64748b', fontSize: '0.95rem' }}>
                  {item.comments ? `"${item.comments}"` : <span style={{ fontStyle: 'italic', opacity: 0.5 }}>No comments provided</span>}
                </p>
              </div>
            ))}
          </div>
